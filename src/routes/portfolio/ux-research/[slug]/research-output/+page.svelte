<script>
	...
	import pubs from "$lib/data/2022-pubs.json";
	...
	$: publication = pubs.find((p) => p.info.key === caseStudy?.publicationKey);
</script>

<section class="ux-detail">
	<p class="eyebrow">UX Research · Output</p>
	<h1>{caseStudy?.title || "Research Output"}</h1>

	{#if publication}
		<div class="pub-card">
			<div class="pub-head">
				<div class="pub-type">
					{#if publication.info.type === "Journal Articles"}
						<div class="bg-violet-200">Journal Paper</div>
					{:else if publication.info.type === "Conference and Workshop Papers"}
						<div class="bg-cyan-300">Conference Paper</div>
					{:else if publication.info.type === "Dissertation"}
				...
				<div class="pub-link group">
					...
					<a href={publication.info.ee} target="_blank" rel="noopener noreferrer">
						<GoLinkExternal />
					</a>
				</div>
			</div>

			<div class="pub-title">{publication.info.title}</div>
			<div class="pub-meta">
				<div>{publication.info.year}</div>
				<div>{publication.info.venue}</div>
			</div>
			<div class="pub-authors">
				{#each publication.info.authors.author as a}
	...
	{:else}
		<p class="body">No linked publication yet.</p>
	{/if}
</section>
