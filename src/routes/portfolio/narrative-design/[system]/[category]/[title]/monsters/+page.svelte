<script>
	...
	const monsterModules = import.meta.glob("/src/lib/narrative-design/monsters/*.svelte", { eager: true });
	const monstersBySlug = Object.fromEntries(
		Object.entries(monsterModules).map(([path, mod]) => [
			path.split("/").pop().replace(".svelte", ""),
			mod.default
		])
	);
	$: MonstersComponent = slug ? monstersBySlug[slug] : null;
</script>
...
<section class="detail-section">
	...
	{#if MonstersComponent}
		<svelte:component this={MonstersComponent} />
	{:else}
...
<style>
	...
	.ability-block {
		...
		border-radius: 14px;
		border: 1px solid rgba(94, 71, 42, 0.3);
